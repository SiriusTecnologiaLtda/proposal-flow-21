...
import {
  ...
  SlidersHorizontal, CalendarRange, Users, X, Check, Search, ChevronDown, ChevronUp, Tag,
...
function getPresetDates(preset: string): { from: string; to: string } {
  ...
  switch (preset) {
    ...
    case "last_month": {
      ...
      return { from: lm.toISOString().substring(0, 10), to: end.toISOString().substring(0, 10) };
    }
    case "this_quarter": {
...
function RoleSelector({
  selectedRole, onChange, allowedRoles,
}: { selectedRole: string; onChange: (role: string) => void; allowedRoles: string[] }) {
  const visibleOptions = ALL_ROLE_OPTIONS.filter((r) => allowedRoles.includes(r.value));
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" className={cn("h-9 gap-2 border-dashed text-xs font-normal", selectedRole !== "all" && "border-primary/40 bg-primary/5 text-primary")}>
          <Users className="h-3.5 w-3.5" />
          {selectedRole === "all" ? "Todos os Níveis" : ALL_ROLE_OPTIONS.find((r) => r.value === selectedRole)?.label || selectedRole.toUpperCase()}
          <ChevronDown className="h-3 w-3 opacity-50" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-48 p-1" align="start">
        <button onClick={() => onChange("all")} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedRole === "all" ? "bg-primary/10 text-primary font-medium" : "text-foreground hover:bg-accent")}>
          Todos os Níveis
        </button>
        {visibleOptions.map((r) => (
          <button key={r.value} onClick={() => onChange(r.value)} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedRole === r.value ? "bg-primary/10 text-primary font-medium" : "text-foreground hover:bg-accent")}>
            {r.label}
          </button>
        ))}
        {selectedRole !== "all" && (
          <div className="border-t border-border mt-1 pt-1">
            <Button variant="ghost" size="sm" className="h-7 w-full text-xs text-muted-foreground" onClick={() => onChange("all")}>
...
// ─── KPI Card ─────────────────────────────────────────────────
function KpiCard({ icon: Icon, label, value, subValue, colorClass, bgClass, delay = 0 }: {
  icon: any; label: string; value: string | number; subValue?: string; colorClass: string; bgClass: string; delay?: number;
}) {
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay }}>
      <Card className="relative overflow-hidden">
        ...
        <CardContent className="relative p-5">
          ...
          <p className={cn("mt-0.5 text-2xl font-bold tracking-tight truncate", colorClass)}>{value}</p>
          {subValue && <p className="mt-1 text-[11px] text-muted-foreground truncate">{subValue}</p>}
        </CardContent>
      </Card>
    </motion.div>
  );
...
// ─── Top 10 Proposals ─────────────────────────────────────────
function Top10Proposals({ proposals, computeNetValue: calcValue, statusMap: sMap }: {
  proposals: any[]; computeNetValue: (p: any) => number | null; statusMap: Record<string, { label: string; className: string }>;
}) {
  const [mode, setMode] = useState<"aberto" | "realizado">("aberto");
  const top10 = useMemo(() => {
    const filtered = mode === "aberto"
      ? proposals.filter((p: any) => p.status === "pendente" || p.status === "proposta_gerada" || p.status === "em_assinatura")
      : proposals.filter((p: any) => p.status === "ganha");
    return filtered.map((p: any) => ({ ...p, _value: calcValue(p) || 0 })).sort((a: any, b: any) => b._value - a._value).slice(0, 10);
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between border-b border-border pb-3">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-sm font-semibold">Top 10 — {mode === "aberto" ? "Em Aberto" : "Realizadas"}</CardTitle>
        </div>
        <div className="flex gap-1">
          <button onClick={() => setMode("aberto")} className={cn("rounded-full border px-3 py-1 text-xs font-medium transition-all", mode === "aberto" ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:text-foreground")}>Em Aberto</button>
          <button onClick={() => setMode("realizado")} className={cn("rounded-full border px-3 py-1 text-xs font-medium transition-all", mode === "realizado" ? "border-success bg-success text-success-foreground" : "border-border text-muted-foreground hover:text-foreground")}>Realizadas</button>
        </div>
      </CardHeader>
      <div className="divide-y divide-border/50">
        {top10.map((proposal: any, i: number) => {
          ...
          const isTop3 = i < 3;
          return (
            <motion.div key={proposal.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}>
              <Link to={`/propostas/${proposal.id}`} className={cn("flex items-center justify-between gap-3 px-4 py-3.5 transition-all hover:bg-accent/50 group", isTop3 && "bg-accent/20")}>
                <div className="flex items-center gap-3.5 min-w-0 flex-1">
                  <span className={cn("flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] font-bold", isTop3 ? `bg-gradient-to-br ${medalColors[i]} text-white shadow-sm` : "bg-muted text-muted-foreground")}>{i + 1}</span>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-foreground truncate group-hover:text-primary transition-colors">{clientName}</p>
                    <p className="text-xs text-muted-foreground truncate">{proposal.number}{esnName ? ` · ${esnName}` : ""}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <span className={cn("text-sm font-bold tabular-nums whitespace-nowrap", isTop3 ? "text-foreground" : "text-muted-foreground")}>{formatCurrency(proposal._value)}</span>
                  <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium whitespace-nowrap ${status.className}`}>{status.label}</span>
                </div>
              </Link>
            </motion.div>
          );
        })}
        {top10.length === 0 && <div className="px-4 py-10 text-center text-sm text-muted-foreground">Nenhuma proposta encontrada para os filtros selecionados.</div>}
      </div>
    </Card>
  );
}
...
// ─── Main Page ────────────────────────────────────────────────
export default function ServiceIndicatorsPage() {
  const { user } = useAuth();
  ...
  const { data: myProfile } = useQuery({
    ...
    enabled: !!user?.id,
    queryFn: async () => {
      const { data, error } = await supabase.from("profiles").select("*, sales_team_member_id").eq("user_id", user!.id).maybeSingle();
  ...
  const { data: salesTargets = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("sales_targets").select("*").eq("year", targetYear);
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const { data: commissionProjections = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("commission_projections").select("*").neq("proposal_status", "cancelada");
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const { data: softwareProposals = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("software_proposals").select("id, status, total_value, proposal_date, esn_id, gsn_id, arquiteto_id, unit_id, client_id, validated_at, segment_id").eq("status", "validated");
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const { data: swProposalItems = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (let i = 0; i < swProposalIds.length; i += 50) {
        ...
        const { data, error } = await supabase.from("software_proposal_items").select("id, software_proposal_id, total_price, catalog_item_id, recurrence").in("software_proposal_id", batch);
        if (error) throw error;
  ...
  const { data: catalogItems = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("software_catalog_items").select("id, category_id");
      if (error) throw error;
      return data || [];
    },
  });
  ...
  const legacyHierarchyScopedIds = useMemo((): string[] | null => {
    if (isEffectiveAdmin) return null;
    if (isDsn) {
    ...
    if (isEsn) return [mySalesTeamId!];
    if (isArquiteto) return [mySalesTeamId!];
    return [];
  }, [isEffectiveAdmin, isDsn, isGsn, isEsn, isArquiteto, mySalesTeamId, salesTeam]);

  const hierarchyScopedIds = useMemo((): string[] | null => {
    if (!FEATURE_FLAGS.useNewScopeDashboard) return legacyHierarchyScopedIds;
  ...
  const combinedMemberFilter = useMemo((): string[] | null => {
    ...
    if (hierarchyScopedIds !== null) result = [...hierarchyScopedIds];
    if (filteredMembersByRole) {
      if (result === null) result = [...filteredMembersByRole];
      else result = result.filter((id) => filteredMembersByRole.includes(id));
    }
    if (unitScopedMemberIds) {
      if (result === null) result = [...unitScopedMemberIds];
      else result = result.filter((id) => unitScopedMemberIds.includes(id));
    }
    if (selectedMemberId !== "all") {
      if (result === null) result = [selectedMemberId];
      else result = result.filter((id) => id === selectedMemberId);
    }
    return result;
  ...
  const myClients = useMemo(() => {
    ...
    if (combinedMemberFilter && combinedMemberFilter.length > 0) {
      return clients.filter((c: any) => combinedMemberFilter.includes(c.esn_id) || combinedMemberFilter.includes(c.gsn_id));
    }
    if (isArquiteto) {
      const clientIdsWithArquiteto = new Set(filteredProposals.filter((p: any) => p.arquiteto_id === mySalesTeamId).map((p: any) => p.client_id));
      return clients.filter((c: any) => clientIdsWithArquiteto.has(c.id));
    }
    if (hierarchyScopedIds) {
      return clients.filter((c: any) => hierarchyScopedIds.includes(c.esn_id) || hierarchyScopedIds.includes(c.gsn_id));
    }
    return [];
  ...
  const penetrationRate = useMemo(() => {
    ...
    const clientsWithWon = new Set(filteredProposals.filter((p: any) => p.status === "ganha" && myClientIds.has(p.client_id)).map((p: any) => p.client_id));
  ...
  const conversionRate = useMemo(() => {
    ...
    return (filteredProposals.filter((p: any) => p.status === "ganha").length / filteredProposals.length) * 100;
  }, [filteredProposals]);
  ...
  const resultadoData = useMemo(() => {
    ...
    const months = MONTH_LABELS.map((label, i) => ({ label, month: i + 1, meta: 0, realizado: 0, previsto: 0 }));

    let relevantTargets = isArquiteto && !isEffectiveAdmin
      ? salesTargets.filter((t: any) => t.esn_id === mySalesTeamId)
      : combinedMemberFilter === null ? salesTargets : salesTargets.filter((t: any) => combinedMemberFilter.includes(t.esn_id));
    ...
    if (selectedRevenueFilter !== "all") {
      ...
      const nrfCategoryIds = categories.filter((c: any) => { const n = normCatName(c.name); return n === "NRF" || n === "RNF"; }).map((c: any) => c.id);
      ...
      if (selectedRevenueFilter === "scs") {
        ...
        const opexCatIds = categories.filter((c: any) => c.cost_classification === "opex" && c.id !== scsCategoryId && c.id !== rrfCategoryId).map((c: any) => c.id);
        ...
        const capexCatIds = categories.filter((c: any) => c.cost_classification === "capex" && !nrfCategoryIds.includes(c.id)).map((c: any) => c.id);
        relevantTargets = relevantTargets.filter((t: any) => capexCatIds.includes(t.category_id));
      }
    }

    for (const t of relevantTargets) {
      if (t.month >= 1 && t.month <= 12) months[t.month - 1].meta += Number(t.amount) || 0;
    }
    ...
    if (includeSCS) {
      const relevantProposals = isArquiteto && !isEffectiveAdmin
        ...
        : combinedMemberFilter === null ? proposals : proposals.filter((p: any) => combinedMemberFilter.includes(p.esn_id) || combinedMemberFilter.includes(p.gsn_id));
    ...
    // Realizado from Software Proposals (non-SCS)
    ...
    if (includeSW && softwareProposals.length > 0) {
      let relevantSW = isArquiteto && !isEffectiveAdmin
        ...
        : combinedMemberFilter === null ? softwareProposals : softwareProposals.filter((sp: any) => combinedMemberFilter.includes(sp.esn_id) || combinedMemberFilter.includes(sp.gsn_id));

      if (selectedUnitId !== "all") {
        relevantSW = relevantSW.filter((sp: any) => {
          if (sp.unit_id === selectedUnitId) return true;
          if (unitScopedMemberIds) return (sp.esn_id && unitScopedMemberIds.includes(sp.esn_id)) || (sp.gsn_id && unitScopedMemberIds.includes(sp.gsn_id));
          return false;
        });
      }
  ...
  const totalMeta = useMemo(() => {
    if (resultadoMode === "ytd") return resultadoData.filter(m => m.month <= currentMonth).reduce((s, m) => s + m.meta, 0);
  ...
  const commissionChartData = useMemo(() => {
    ...
    const MONTH_LABELS = ["Jan", "Fev", "Mar", "Abr", "Mai", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dez"];
    const months = MONTH_LABELS.map((label, i) => ({ label, month: i + 1, realizada: 0, prevista: 0 }));
    ...
    if (isArquiteto && !isEffectiveAdmin) {
      ...
      for (const p of myProposals) {
        ...
        const dateStr = p.status === "ganha" ? (p.expected_close_date || (p.updated_at || "").substring(0, 10)) : (p.expected_close_date || "");
        if (!dateStr) continue;
        const year = Number(dateStr.substring(0, 4));
        const month = Number(dateStr.substring(5, 7));
        if (year !== targetYear || month < 1 || month > 12) continue;
        if (p.status === "ganha") { months[month - 1].realizada += commValue; }
        else { if (dateStr <= today) continue; months[month - 1].prevista += commValue; }
      }
    } else {
      ...
      for (const cp of relevant) {
        ...
        if (dueDate <= today) {
          if (cp.proposal_status === "ganha") months[month - 1].realizada += commValue;
        } else {
          if (cp.proposal_status === "ganha") months[month - 1].realizada += commValue;
          else months[month - 1].prevista += commValue;
        }
      }
    }
    return months;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Indicadores de Serviços</h1>
          <p className="text-sm text-muted-foreground">Oportunidades, análise de resultado e indicadores de performance</p>
        </div>
        <Button asChild>
      ...
      <Card className="overflow-hidden">
        <button type="button" onClick={() => setFiltersOpen(!filtersOpen)} className="flex w-full items-center gap-3 border-b border-border bg-accent/30 px-4 py-2.5 text-left transition-colors hover:bg-accent/50">
          <div className="flex items-center gap-2 text-muted-foreground">
            <SlidersHorizontal className="h-4 w-4" />
            <span className="text-xs font-semibold uppercase tracking-wider">Filtros</span>
          </div>
          {activeFilters > 0 && (
            <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex h-5 w-5 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground">{activeFilters}</motion.span>
          )}
          <div className="flex-1" />
          {activeFilters > 0 && (
            <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground hover:text-destructive" onClick={(e) => { e.stopPropagation(); handlePreset("this_year"); setSelectedRoleFilter("all"); setSelectedUnitId("all"); setSelectedRevenueFilter("all"); setSelectedCategoryId("all"); setSelectedMemberId("all"); }}>
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
              <CardContent className="p-4">
                <div className="flex flex-wrap items-start gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {PERIOD_PRESETS.map((p) => (
                        <button key={p.value} onClick={() => handlePreset(p.value)} className={cn("rounded-full border px-3 py-1 text-xs font-medium transition-all", periodPreset === p.value ? "border-primary bg-primary text-primary-foreground shadow-sm" : "border-border bg-background text-muted-foreground hover:border-primary/40 hover:text-foreground")}>{p.label}</button>
                      ))}
                    </div>
                    <AnimatePresence>
                      {periodPreset === "custom" && (
                  ...
                  {showRoleFilter && <div className="hidden h-16 w-px self-center bg-border md:block" />}
                  {showRoleFilter && (
                    <div className="space-y-2">
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        <Users className="h-3.5 w-3.5" />
                      ...
                      <RoleSelector selectedRole={selectedRoleFilter} onChange={setSelectedRoleFilter} allowedRoles={allowedRoleFilterOptions} />
                    </div>
                  )}
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className={cn("h-9 gap-2 border-dashed text-xs font-normal", selectedUnitId !== "all" && "border-primary/40 bg-primary/5 text-primary")}>
                          <Building2 className="h-3.5 w-3.5" />
                          {selectedUnitId === "all" ? "Todas as unidades" : units.find((u) => u.id === selectedUnitId)?.name || "Unidade"}
                          <ChevronDown className="h-3 w-3 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-64 p-0" align="start">
                        <div className="max-h-64 overflow-auto p-1">
                          <button onClick={() => setSelectedUnitId("all")} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedUnitId === "all" ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                            {selectedUnitId === "all" && <Check className="h-3.5 w-3.5" />}
                            <span className="font-medium">Todas as unidades</span>
                          </button>
                          {allowedUnits.map((u) => (
                            <button key={u.id} onClick={() => setSelectedUnitId(u.id)} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedUnitId === u.id ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className={cn("h-9 gap-2 border-dashed text-xs font-normal", selectedRevenueFilter !== "all" && "border-primary/40 bg-primary/5 text-primary")}>
                          <Repeat className="h-3.5 w-3.5" />
                          {selectedRevenueFilter === "all" ? "Todas as receitas" : REVENUE_FILTER_OPTIONS.find((r) => r.value === selectedRevenueFilter)?.label || selectedRevenueFilter}
                          <ChevronDown className="h-3 w-3 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-56 p-0" align="start">
                        <div className="max-h-64 overflow-auto p-1">
                          <button onClick={() => setSelectedRevenueFilter("all")} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedRevenueFilter === "all" ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                          ...
                          {REVENUE_FILTER_OPTIONS.map((r) => (
                            <button key={r.value} onClick={() => setSelectedRevenueFilter(r.value)} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedRevenueFilter === r.value ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                        ...
                        {selectedRevenueFilter !== "all" && (
                          <div className="border-t border-border p-1">
                            <Button variant="ghost" size="sm" className="h-7 w-full text-xs text-muted-foreground" onClick={() => setSelectedRevenueFilter("all")}>
                              <X className="mr-1 h-3 w-3" /> Limpar
                            </Button>
                          </div>
                        )}
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className={cn("h-9 gap-2 border-dashed text-xs font-normal", selectedCategoryId !== "all" && "border-primary/40 bg-primary/5 text-primary")}>
                          <Tag className="h-3.5 w-3.5" />
                          {selectedCategoryId === "all" ? "Todas as categorias" : categories.find((c) => c.id === selectedCategoryId)?.name || selectedCategoryId}
                          <ChevronDown className="h-3 w-3 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-56 p-0" align="start">
                        <div className="max-h-64 overflow-auto p-1">
                          <button onClick={() => setSelectedCategoryId("all")} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedCategoryId === "all" ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                            {selectedCategoryId === "all" && <Check className="h-3.5 w-3.5" />}
                            <span className="font-medium">Todas as categorias</span>
                          </button>
                          {categories.map((c) => (
                            <button key={c.id} onClick={() => setSelectedCategoryId(c.id)} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedCategoryId === c.id ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                        ...
                        {selectedCategoryId !== "all" && (
                          <div className="border-t border-border p-1">
                            <Button variant="ghost" size="sm" className="h-7 w-full text-xs text-muted-foreground" onClick={() => setSelectedCategoryId("all")}>
                              <X className="mr-1 h-3 w-3" /> Limpar
                            </Button>
                          </div>
                        )}
                  ...
                  <div className="space-y-2">
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" className={cn("h-9 gap-2 border-dashed text-xs font-normal max-w-[220px]", selectedMemberId !== "all" && "border-primary/40 bg-primary/5 text-primary")}>
                          <User className="h-3.5 w-3.5 shrink-0" />
                          <span className="truncate">{selectedMemberId === "all" ? "Todos os membros" : allowedMembers.find((m) => m.id === selectedMemberId)?.name || "Membro"}</span>
                          <ChevronDown className="h-3 w-3 opacity-50" />
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-64 p-0" align="start">
                        <div className="max-h-64 overflow-auto p-1">
                          <button onClick={() => setSelectedMemberId("all")} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedMemberId === "all" ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                            {selectedMemberId === "all" && <Check className="h-3.5 w-3.5" />}
                            <span className="font-medium">Todos os membros</span>
                          </button>
                          {allowedMembers.map((m) => (
                            <button key={m.id} onClick={() => setSelectedMemberId(m.id)} className={cn("flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-xs transition-colors", selectedMemberId === m.id ? "bg-primary/10 text-primary" : "text-foreground hover:bg-accent")}>
                              {selectedMemberId === m.id && <Check className="h-3.5 w-3.5" />}
                              <span className="font-medium">{m.name}</span>
                              <span className="text-muted-foreground ml-auto">{m.role?.toUpperCase()}</span>
                            </button>
                          ))}
                        </div>
                        {selectedMemberId !== "all" && (
                          <div className="border-t border-border p-1">
                            <Button variant="ghost" size="sm" className="h-7 w-full text-xs text-muted-foreground" onClick={() => setSelectedMemberId("all")}>
                              <X className="mr-1 h-3 w-3" /> Limpar
                            </Button>
                          </div>
                        )}
      ...
      <Tabs defaultValue="propostas" className="space-y-6">
        ...
        <TabsContent value="propostas" className="space-y-6">
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-6">
            <KpiCard icon={TrendingUp} label="Propostas Ganhas" value={wonProposals.length} colorClass="text-success" bgClass="bg-success/15" delay={0} />
            <KpiCard icon={Target} label="Valor Ganho" value={formatCurrency(wonValue)} colorClass="text-success" bgClass="bg-success/15" delay={0.05} />
            <KpiCard icon={TrendingDown} label="Propostas Perdidas" value={lostProposals.length} colorClass="text-destructive" bgClass="bg-destructive/15" delay={0.1} />
            <KpiCard icon={Target} label="Valor Perdido" value={formatCurrency(lostValue)} colorClass="text-destructive" bgClass="bg-destructive/15" delay={0.15} />
            <KpiCard icon={FileText} label="Propostas Pendentes" value={pendingProposals.length} colorClass="text-warning" bgClass="bg-warning/15" delay={0.2} />
            <KpiCard icon={Target} label="Valor Pendente" value={formatCurrency(pendingValue)} colorClass="text-warning" bgClass="bg-warning/15" delay={0.25} />
          </div>
                        ...
                        <div className="rounded-lg border border-border bg-background p-3 shadow-md">
                          ...
                          {data?.ganhas > 0 && (
                            <div className="flex items-center justify-between gap-4 text-sm">
                              <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm" style={{ background: "hsl(var(--success))" }} />Ganhas ({data.ganhas})</span>
                              <span className="font-medium">{data.ganhasValor.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
                            </div>
                          )}
                          {data?.perdidas > 0 && (
                            <div className="flex items-center justify-between gap-4 text-sm">
                              <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-sm" style={{ background: "hsl(var(--destructive))" }} />Perdidas ({data.perdidas})</span>
                              <span className="font-medium">{data.perdidasValor.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
                            </div>
                          )}
                          {(!data?.ganhas && !data?.perdidas) && <p className="text-xs text-muted-foreground">Sem dados</p>}
                        </div>
                      );
                    }}
                  />
        ...
        <TabsContent value="resultado" className="space-y-6">
          <div className="flex items-center justify-end gap-1.5">
            ...
            {(["ytd", "anual"] as const).map((mode) => (
              <button key={mode} onClick={() => setResultadoMode(mode)} className={cn("rounded-full border px-3 py-1 text-xs font-medium transition-all", resultadoMode === mode ? "border-primary bg-primary text-primary-foreground shadow-sm" : "border-border bg-background text-muted-foreground hover:border-primary/40 hover:text-foreground")}>
                {mode === "ytd" ? "YTD" : "Anual"}
              </button>
            ))}
          </div>
        ...
        <TabsContent value="performance" className="space-y-6">
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-5">
            <KpiCard icon={Users} label="Quantidade de Clientes" value={myClients.length} subValue={role === "admin" ? "todos os clientes" : mySalesTeamMember ? `clientes do ${mySalesTeamMember.role?.toUpperCase()}` : "vincule seu perfil ao time de vendas"} colorClass="text-primary" bgClass="bg-primary/15" delay={0} />
            <KpiCard icon={Percent} label="Penetração" value={`${penetrationRate.toFixed(1)}%`} subValue="clientes com proposta ganha" colorClass="text-success" bgClass="bg-success/15" delay={0.05} />
            <KpiCard icon={UserCheck} label="Taxa de Conversão" value={`${conversionRate.toFixed(1)}%`} subValue="ganhas / total de propostas" colorClass="text-primary" bgClass="bg-primary/15" delay={0.1} />
            <KpiCard icon={BarChart3} label="Ticket Médio" value={formatCurrency(avgTicket)} subValue="por proposta ganha" colorClass="text-primary" bgClass="bg-primary/15" delay={0.15} />
            <KpiCard icon={Clock} label="Tempo Médio" value={`${avgLifecycleDays}d`} subValue="abertura → ganho" colorClass="text-warning" bgClass="bg-warning/15" delay={0.2} />
          </div>
