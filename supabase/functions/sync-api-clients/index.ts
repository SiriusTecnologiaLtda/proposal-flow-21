...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { integrationId, triggerType, syncLogId } = await req.json();
    if (!integrationId) {
    ...
    // Reconcile stale running syncs for this integration
    ...
    // Concurrency check: block if there's an active sync with recent heartbeat
    if (!syncLogId) {
    ...
    if (logId) {
      ...
      if (existingLog) {
        currentOffset = existingLog.current_offset || 0;
        pagesProcessed = existingLog.pages_processed || 0;
        recordsFetched = existingLog.records_fetched || 0;
        inserted = existingLog.inserted || 0;
        updated = existingLog.updated || 0;
        errors = existingLog.errors || 0;
      }
    } else {
    ...
    // Helper: update heartbeat + counters
    ...
    // Helper: process a batch of records via upsert
    const processRecords = async (records: any[]) => {
      ...
      // Batch upsert using code + store_code
      for (const row of mapped) {
        try {
          ...
          if (existing) {
            await admin.from("clients").update(row).eq("id", existing.id);
            updated++;
          } else {
    ...
    const fetchPage = async (): Promise<{ records: any[]; url: string; curlCmd: string; httpStatus: number; responsePreview: string; duration: number }> => {
      let url: string;
      ...
      return { records, url, curlCmd, httpStatus: apiRes.status, responsePreview: responseText.substring(0, 500), duration };
    };

    // Main processing: process pages in a loop, but with time budget
    const MAX_EXECUTION_MS = 50_000; // 50s budget (edge functions have ~60s limit)
    ...
    try {
      while (hasMore) {
        ...
        if (Date.now() - startExecution > MAX_EXECUTION_MS) {
          ...
          // Fire next invocation
          const nextBody = JSON.stringify({ integrationId, triggerType: triggerType || "manual", syncLogId: logId });
          ...
          return new Response(JSON.stringify({ status: "continuing", logId, pagesProcessed, inserted, updated, errors }), {
        ...
        try {
          ...
          return new Response(JSON.stringify({ error: err.message, logId }), {
            status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      ...
      return new Response(JSON.stringify({ inserted, updated, errors, logId, pagesProcessed }), {
        status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
